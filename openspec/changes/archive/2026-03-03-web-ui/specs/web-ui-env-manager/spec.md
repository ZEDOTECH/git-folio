...
`GET /api/env` SHALL 讀取專案根目錄的 `.env` 檔案，解析所有 key=value 對，並回傳 JSON。Token 類欄位（含 `PAT`、`KEY`、`TOKEN` 的 key）SHALL 遮罩：僅顯示前 4 + 後 4 字元，中間以 `●●●●●●●●` 取代；若值長度 ≤ 8 則完全遮罩。非 token 欄位（如 `OPENAI_MODEL`、`AUTHOR_NAME`、`SITE_URL`）回傳原始值。
