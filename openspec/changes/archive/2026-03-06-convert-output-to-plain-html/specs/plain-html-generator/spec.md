...
每個 HTML 頁面的 `<head>` SHALL 包含 `<script src="https://cdn.tailwindcss.com"></script>`。自訂元件樣式（`.card`、`.mono`、`.tag`、`.btn-ghost`、`.section-title`、`.section-subtitle`）SHALL 以 Tailwind `plugin` config 定義在同一個 inline `<script type="text/tailwind-config">` 或 inline `<style>` block 中。
