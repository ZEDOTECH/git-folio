...
- **THEN** 說明 SHALL 包含 Skip AI enrichment、No cache 兩個選項
- **THEN** 說明 SHALL NOT 包含「Include private repos」或「Skip AI descriptions for private repos」（此等選項不存在於 UI）
...
- **THEN** README SHALL 提及 OPENAI_MODEL 欄位（預設 `gpt-5-mini`，可自訂模型）
