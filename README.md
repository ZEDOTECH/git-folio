...
從 GitHub 自動生成 AI 驅動的個人作品集網站。抓取所有 repos、用 GPT-5-mini 撰寫描述、生成完整可部署的純靜態 HTML 作品集。
...
每次 generate 完後，執行以下指令即可將 `output/` 推送到 `gh-pages` branch：
...
4. 點 Save，幾分鐘後即可在 `https://<username>.github.io/<repo-name>` 看到作品集

**之後每次更新流程：**

```bash
# 1. 重新 generate（Web UI 或 CLI 皆可）
